/// @file ocp_stc_landing.hpp
/// @brief CT-cSTC receding-horizon landing OCP ("stc_landing").
///
/// Ported from ALIPDDP-main/problem_examples/STC/quad_landing_rh_clean_stc.cpp
/// (the validated "rh clean stc" benchmark). Relative-frame landing on a moving
/// target with a continuous-time compound state-triggered constraint (CT-cSTC)
/// accumulator carried as an extra state row:
///
///   state   = [13 physical | IDX_DT cumulative time | IDX_CTCS_Y accumulator]
///   control = [fz, Mx, My, Mz | IDX_THETA free per-step dt]
///
/// The accumulator integrates trigger^2 * violation^2 products (landing
/// speed/tilt/omega/glideslope, staging hold, LOS cone, thrust bands) through
/// the same RK4 map as the dynamics and is driven to zero by a terminal EQ
/// constraint (default) or per-step increment inequality (SZMUK_CTCS_MODE).
///
/// All SZMUK_* environment tunables from the benchmark are preserved; export
/// them before launching the planner to retune without rebuilding.
///
/// Warm-start note: the planner core's makeXshifted() does a plain index
/// shift, so create() rebases IDX_DT and IDX_CTCS_Y against the new horizon
/// start (prev_X[0] after the shift == the previously executed node), exactly
/// as the benchmark's makeXshifted did. The core carries the y-row through the
/// replayer untouched (state_dim=13, DT at 13, y rides along at 14).
#pragma once
...
static const double THETA_PHASE0_DEG   = envOrQ("SZMUK_TILT_PHASE0_DEG", 15.0);
static const double OMEGA_PHASE0_MAX   = envOrQ("SZMUK_OMEGA_PHASE0_MAX", 0.50);
static const double SPD_PHASE0_MAX     = envOrQ("SZMUK_SPD_PHASE0_MAX", 3.0);
static const double THETA_STC_DEG      = envOrQ("SZMUK_TILT_STC_DEG", 2.0);
static const double ALPHA_THETA_STC    = envOrQ("SZMUK_ALPHA_THETA_STC", 1.0);
static const double OMEGA_STC_MAX      = envOrQ("SZMUK_OMEGA_STC_MAX", 0.05);
...
static const double STAGE_RADIUS       = envOrQ("SZMUK_STAGE_RADIUS", 0.55);
static const double STAGE_TRIGGER_SCALE = envOrQ("SZMUK_STAGE_TRIGGER_SCALE", 0.11);
...
inline std::string ctcsEnforcementMode() {
    const char* env = std::getenv("SZMUK_CTCS_MODE");
    return env ? std::string(env) : std::string("terminal_eq");
}
...
// Accumulator rescaling (rocket Y_SCALE pattern): the solver carries
// y' = y / CTCS_Y_SCALE and the step budget eps' = eps / CTCS_Y_SCALE, so the
// y-row curvature stops dwarfing the physical states; the zero set is unchanged.
static const double CTCS_Y_SCALE = [] {
    const double s = envOrQ("SZMUK_CTCS_Y_SCALE", 1.0);
    return s > 0.0 ? s : 1.0;
}();
static const double CTCS_STEP_EPS = envOrQ("SZMUK_CTCS_STEP_EPS", 1e-5);
...
// ── Relative dynamics with constant-accel target prediction + y-row ──────────
template<typename Scalar>
...
public:
    ...
    // 15-dim propagate for warm-start rollouts (tracks IDX_DT and CT-cSTC y).
    Eigen::VectorXd propagate15(const Eigen::VectorXd& x15,
                                const Eigen::VectorXd& u_phys,
                                double Th) const {
        double tabs = x15(IDX_DT);
        ...
        xn15(IDX_CTCS_Y) = x15(IDX_CTCS_Y) +
                           (Th/6.0)*(h1 + 2.0*h2 + 2.0*h3 + h4);
        return xn15;
    }
};
...
// CT-cSTC terminal constraint: y_N = 0 (augmented Lagrangian equality).
template<typename Scalar>
...
// CT-cSTC per-step accumulator increment constraint:
//   y_{k+1}(x_k,u_k) - y_k <= eps
// using the same augmented RK4 map as the dynamics.
template<typename Scalar>
class CtcsAccumulatorStepIneqCon : public StageConstraintBase<Scalar> {
public:
    CtcsAccumulatorStepIneqCon(
    ...
    Vector<Scalar> c(const Vector<Scalar>& x, const Vector<Scalar>& u) const override {
        Vector<Scalar> out(1);
        out(0) = dyn_->f(x, u)(IDX_CTCS_Y) - x(IDX_CTCS_Y) - eps_;
        return out;
    }
    ...
    Matrix<Scalar> cx(const Vector<Scalar>& x, const Vector<Scalar>& u) const override {
        ...
        for (int i = 0; i < x.size(); ++i) {
            ...
            xp(i) += Scalar(eps);
            xm(i) -= Scalar(eps);
            J(0, i) = (c(xp, u)(0) - c(xm, u)(0)) / Scalar(2.0 * eps);
        }
        return J;
    }
    ...
    Matrix<Scalar> cu(const Vector<Scalar>& x, const Vector<Scalar>& u) const override {
        ...
        for (int i = 0; i < u.size(); ++i) {
            ...
            up(i) += Scalar(eps);
            um(i) -= Scalar(eps);
            J(0, i) = (c(x, up)(0) - c(x, um)(0)) / Scalar(2.0 * eps);
        }
        return J;
    }
...
inline Eigen::VectorXd ensureCtcsStateSize(const Eigen::VectorXd& x) {
    ...
    std::shared_ptr<ConstantAccelPredictor> predictor = nullptr)
{
    ...
    const std::string mode = ctcsEnforcementMode();
    // The constraint sees the SCALED increment, so the physical budget
    // CTCS_STEP_EPS is divided by CTCS_Y_SCALE (dy' <= eps' <=> dy_phys <= eps_phys).
    auto cstep = std::make_shared<CtcsAccumulatorStepIneqCon<double>>(
    ...
    } else if (mode != "step_ineq" && mode != "off") {
        std::cerr << "[WARN] Unknown SZMUK_CTCS_MODE=\"" << mode
                  << "\"; expected terminal_eq, step_ineq, or off. Using off.\n";
    }
    ...
    // Rebase the shifted warm-start references: IDX_DT and IDX_CTCS_Y are
    // per-solve accumulators, but the core's makeXshifted() does a plain index
    // shift, so prev_X[0] here is the previously executed node carrying the
    // old horizon's t0/y0. Subtract them (benchmark makeXshifted equivalent)
    // and pin the first reference to the new initial state.
    std::vector<Eigen::VectorXd> Xw = prev_X;
    if (!Xw.empty()) {
        const Eigen::VectorXd x_base = ensureCtcsStateSize(Xw[0]);
        const double t0_shift = x_base(IDX_DT);
        const double y0_shift = x_base(IDX_CTCS_Y);
        bool finite = true;
        for (auto& x : Xw) {
            x = ensureCtcsStateSize(x);
            if (!x.allFinite()) { finite = false; break; }
            x(IDX_DT)     = std::max(0.0, x(IDX_DT)     - t0_shift);
            x(IDX_CTCS_Y) = std::max(0.0, x(IDX_CTCS_Y) - y0_shift);
        }
        if (finite) {
            Xw[0] = x0ss;
        } else {
    ...
    for (int k = 0; k < N; ++k) {
        ...
        if (static_cast<int>(prev_U.size()) >= N) {
            ...
            // Geometric straight-line seed toward the target frame origin.
            ...
            const double tabs_ws = sim15(IDX_DT);
        ...
        Eigen::VectorXd sim15_next = dyn->propagate15(sim15, uk.segment(0, NU), uk(IDX_THETA));
...
inline OCPDescriptor descriptor() {
    ...
    d.create = [](const OCPCreateArgs& a) {
        ...
        if (a.extra.has_value()) {
            try { ex = std::any_cast<StcLandingExtra>(a.extra); }

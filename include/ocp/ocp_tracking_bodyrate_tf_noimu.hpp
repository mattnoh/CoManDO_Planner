/// @file ocp_tracking_bodyrate_tf_noimu.hpp
/// @brief Target-frame body-rate tracking OCP, no IMU augmented states (10D).
///
/// State (10D physical + DT = 11D):
...
///   x[10]   — DT    : cumulative time (variable timestep)
///
/// Control (5D):
///   u[0]   — T      : specific thrust [m/s²]
///   u[1:4] — ω_B    : body angular rates [rad/s]
///   u[4]   — Theta  : time segment [s]
///
/// DroneOdomMode: TargetFrameRelative — reads from /drone/target_frame_odom
/// The target_publisher "cheating estimator" computes this from world-frame
/// ground-truth ROS2 topics and publishes as if from an onboard relative estimator.
#pragma once
...
static constexpr double GS_TAN = 1.0;   // tan(45°)
...
// SOC glideslope in target frame N (yaw-only target → N aligned with world-z)
// Enforces drone above target: tan_gs * (p_z + dz) >= ||(p_x, p_y)||
template<typename Scalar>
    ...
    double th_init = TH_INIT)
{
    ...
    auto dyn = std::make_shared<Quad6DOFTargetFrameBodyRateSimple<double>>();
...
inline OCPDescriptor descriptor() {
    OCPDescriptor d;
    d.name = "tracking_bodyrate_tf_noimu";
    d.dt = TH_INIT;
    ...
    d.make_hover_state = [](const Eigen::VectorXd& x) -> Eigen::VectorXd {
        ...
        if (x.size() >= 10) h.head(10) = x.head(10);
        return h;
    };

    // Passthrough — state from /drone/target_frame_odom is already in target frame
    d.transform_state = [](const Eigen::VectorXd& x_drone, const TargetSnapshot&) {
        Eigen::VectorXd x(NX);
        x.setZero();
        const int n = std::min(static_cast<int>(x_drone.size()), NX);
        x.head(n) = x_drone.head(n);
        return x;
    };

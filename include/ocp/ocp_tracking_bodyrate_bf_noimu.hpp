/// @file ocp_tracking_bodyrate_bf_noimu.hpp
/// @brief Body-frame relative body-rate tracking OCP, no IMU augmented states (10D).
///
...
namespace TrackingBodyrateBfNoImuOCP {
...
// Glideslope in inertial frame via body-frame state
template<typename Scalar>
    ...
    double th_init = TH_INIT)
{
    ...
    auto dyn = std::make_shared<Quad6DOFBodyFrameBodyRateSimple<double>>();
    ...
    x0ss.setZero();
    const int n = std::min(static_cast<int>(x0.size()), NX);
    x0ss.head(n) = x0.head(n);
...
inline OCPDescriptor descriptor() {
    OCPDescriptor d;
    d.name = "tracking_bodyrate_bf_noimu";
    d.dt = TH_INIT;
    ...
    d.state_names = {"px_body","py_body","pz_body","vdx","vdy","vdz","qw","qx","qy","qz","DT"};
    ...
    // Passthrough — state from /drone/body_relative_odom is already in body frame

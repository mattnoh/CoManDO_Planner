...
/// @brief Acados MPC implementation — wraps generated C solver code.
...
// ── Include the generated acados C header ────────────────────────────────────
extern "C" {
...
AcadosMPC::AcadosMPC(const Config& config)
    : config_(config)
{
    initSolver();
}
...
void AcadosMPC::initSolver()
{
    ...
    if (status != 0) {
        quadrotor_acados_free_capsule(capsule_);
        capsule_ = nullptr;
        throw runtime_error("AcadosMPC: quadrotor_acados_create() failed, status=" + to_string(status));
    }

    initialized_ = true;

    // Set the reference once (updated by setTerminalState if needed)
    setReference();

    cout << "[AcadosMPC] Solver initialized: N=" << QUADROTOR_N
         << " nx=" << QUADROTOR_NX << " nu=" << QUADROTOR_NU
         << " nh=" << QUADROTOR_NH << endl;
}
...
void AcadosMPC::setInitialState(const Eigen::VectorXd& x0)
{
    ...
    ocp_nlp_config* config  = quadrotor_acados_get_nlp_config(capsule_);
    ocp_nlp_dims* dims      = quadrotor_acados_get_nlp_dims(capsule_);

    // Stage 0 has box constraints on x: lbx0 = ubx0 = x0
    // The index set idxbx0 was set during code generation to [0..12].
    double x0_data[QUADROTOR_NX];
    for (int i = 0; i < QUADROTOR_NX; ++i) {
        x0_data[i] = x0(i);
    }
    ocp_nlp_constraints_model_set(config, dims, nlp_in, 0, "lbx", x0_data);
    ocp_nlp_constraints_model_set(config, dims, nlp_in, 0, "ubx", x0_data);
}

// ═══════════════════════════════════════════════════════════════════════════════
//  Set cost reference for all stages
// ═══════════════════════════════════════════════════════════════════════════════
...
void AcadosMPC::setReference()
{
    ...
    x_ref(6) = 1.0;  // upright quaternion

    // If terminal_state is set, use it as reference
    if (config_.terminal_state.size() == 13) {
        x_ref = config_.terminal_state;
    }
    ...
    // Stage reference: y_ref = [x_ref; u_ref]  (19-dim)
    ...
    for (int k = 0; k < N; ++k) {
        ocp_nlp_cost_model_set(config, dims, nlp_in, k, "yref", y_ref);
    }

    // Terminal reference: y_ref_e = x_ref  (13-dim)
    double y_ref_e[QUADROTOR_NYN];
    for (int i = 0; i < 13; ++i) y_ref_e[i] = x_ref(i);
    ocp_nlp_cost_model_set(config, dims, nlp_in, N, "yref", y_ref_e);
}

// ═══════════════════════════════════════════════════════════════════════════════
//  Shift warm-start forward by n_shift steps
// ═══════════════════════════════════════════════════════════════════════════════
...
void AcadosMPC::shiftWarmStart()
{
    ...
    // Shift X: x_warm[k] = x_prev[min(k + n_shift, N)]
    for (int k = 0; k <= N; ++k) {
        int src = min(k + n_shift, N);
        double x_data[QUADROTOR_NX];
        for (int i = 0; i < QUADROTOR_NX; ++i) {
            x_data[i] = prev_X_[src](i);
        }
        ocp_nlp_out_set(config, dims, nlp_out, k, "x", x_data);
    }

    // Shift U: u_warm[k] = u_prev[min(k + n_shift, N-1)]
    for (int k = 0; k < N; ++k) {
        int src = min(k + n_shift, N - 1);
        double u_data[QUADROTOR_NU];
        for (int i = 0; i < QUADROTOR_NU; ++i) {
            u_data[i] = prev_U_[src](i);
        }
        ocp_nlp_out_set(config, dims, nlp_out, k, "u", u_data);
    }
}
...
AcadosMPC::Result AcadosMPC::solve(const Eigen::VectorXd& current_state)
{
    ...
    if (!capsule_ || !initialized_) {
        cerr << "[AcadosMPC] Solver not initialized\n";
        return result;
    }
    ...
    try {
        ...
        // Shift warm-start from previous solution
        ...
        if (status != 0 && status != 2) {
            cerr << "[AcadosMPC] Solver failed, status=" << status << endl;
            return result;
        }
        ...
        for (int k = 0; k <= N; ++k) {
            ...
            double x_data[QUADROTOR_NX];
            ocp_nlp_out_get(config, dims, nlp_out, k, "x", x_data);
            for (int i = 0; i < QUADROTOR_NX; ++i) {
        ...
        for (int k = 0; k < N; ++k) {
            ...
            double u_data[QUADROTOR_NU];
            ocp_nlp_out_get(config, dims, nlp_out, k, "u", u_data);
            for (int i = 0; i < QUADROTOR_NU; ++i) {
        ...
        cerr << "[AcadosMPC] Exception: " << e.what() << endl;
...
void AcadosMPC::setTerminalState(const Eigen::VectorXd& terminal)
{
    if (terminal.size() == 13) {
        ...
        has_prev_solution_ = false;  // force fresh start with new reference
        setReference();               // update cost reference in solver
    }
}

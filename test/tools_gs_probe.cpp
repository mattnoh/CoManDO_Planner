...
int main(int argc,char**argv){
    ...
    int ok=0; double lat_sum=0, gs_worst=0, tilt_worst=0, lat_worst=0, ms_sum=0, ms_max=0;
    for(int k=0;k<8;++k){
        ...
        double lat_at_floor=-1,gsm=0,tlt=0;
    ...
    printf("RESULT ok=%d/8  gs_max=%.1f  tilt_max=%.1f  ms_avg=%.0f ms_max=%.0f %s\n",
           ok, gs_worst, tilt_worst, ok?ms_sum/ok:0.0, ms_max,
           (ms_max<=250.0&&ok>=7)?"[OK-RT]":"");
    return 0;
}
